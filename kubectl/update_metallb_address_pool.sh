...
# Description: Checks the current external IP address and updates
#              the specified MetalLB IPAddressPool resource in Kubernetes.
# ============================================

# Function to display usage
usage() {
  echo "Usage: POOL_NAME=<pool-name> POOL_NAMESPACE=<namespace> ./update_metallb_ip_pool.sh"
  echo "Example: POOL_NAME=router-orange POOL_NAMESPACE=metallb ./update_metallb_ip_pool.sh"
  exit 1
}

# Check if required environment variables are set
if [[ -z "$POOL_NAME" || -z "$POOL_NAMESPACE" ]]; then
...
if [[ -z "$CURRENT_IP" ]]; then
  echo "$(date '+%Y-%m-%d %H:%M:%S') [ERROR]: Failed to retrieve external IP."
  exit 1
fi
